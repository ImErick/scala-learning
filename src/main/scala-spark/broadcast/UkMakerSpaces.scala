...
object UkMakerSpaces {
  // no entiendo bien el proposito de las variables broadcast
  def main(args: Array[String]): Unit = {
    Logger.getLogger("org").setLevel(Level.ERROR)
    val conf = new SparkConf().setAppName("UkMakerSpaces").setMaster("local[*]")
    val sparkContext = new SparkContext(conf)

    val postCodeMap = sparkContext.broadcast(loadPostCodeMap())
    val makerSpaceRdd = sparkContext.textFile("files/uk-makerspaces-identifiable-data.csv")
  ...
  def loadPostCodeMap(): Map[String, String] = {
    Source.fromFile("files/uk-postcode.csv").getLines.map(line => {
      val splits = line.split(Utils.COMMA_DELIMITER, -1)
      splits(0) -> splits(7)
    }).toMap
  }
